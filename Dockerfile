...
RUN apt-get update \
    && apt-get install -y --no-install-recommends gcc g++ nginx libglib2.0-0 libsm6 libxext6 libxrender-dev libgl1-mesa-dev \
...
RUN poetry install --no-interaction --no-ansi --no-dev \
    ...
    && apt-get remove --auto-remove --purge -y gcc g++ \
    && apt-get clean \
